...
export const apiOverride = async <T, U = unknown>(
  ...
  const requestUrl_updated = window.location.protocol+'//'+window.location.hostname+requestUrl;
  //console.log('requestUrl_updated='+requestUrl_updated);
  const response = await fetch(`${requestUrl_updated}${new URLSearchParams(params)}`, {
    method,
